...
K_MEM_SLAB_DEFINE(mutex_slab, sizeof(struct k_mutex), DPL_MAX_MUTEXES, MEM_ALIGN);

static struct k_mutex *dpl_mutex_pool_alloc()
{
    struct k_mutex *mutex_ptr = NULL;

    if (k_mem_slab_alloc(&mutex_slab, (void **)&mutex_ptr, K_NO_WAIT) < 0)
    {
        /*
...
uintptr_t MutexP_lock(MutexP_Handle handle)
{
    ...
    __ASSERT(retval == 0, "MutexP_lock: retval: %d\r\n", retval);
